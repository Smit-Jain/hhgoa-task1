...
export default function DynamicBackground() {
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      const sandY = skyHeight + seaHeight; // Y = 56% down
      ...
      const baseSunRadius = Math.min(width, height) * 0.18; // Size matching the photo
      ...
      drawDetailedTree(Math.max(60, width * 0.05), height - 45, 1.8, wind);

      // Surfboards on the right side
      const boardBaseX = Math.min(width - 26, width * 0.95 + 14);
      const boardBaseY = height - 265;
